...
- `--nav-height`: lo setea el Navbar con `ResizeObserver`; secciones usan `padding-top: max(1rem, var(--nav-height, 64px))`. Excepción `MyAccountPage`: `pt-0 md:pt-[max(1rem,var(--nav-height))]` para que hero quede pegado arriba en mobile (sin gap).
- **Badge "Última edición" solo en la última:** `edition_languages.badge` debe vivir únicamente en la edición con `max(edition_number)` / `featured=true` (hoy #4 id=11). Duplicarlo en #3 causaba pill doble en “Tus tomos”/hero/detalle (corregido Ago-2026: `update edition_languages set badge=null where edition_id=3`).
...
- **Layout mobile mi-cuenta + detalle (Ago-2026):** Hero `MyAccountPage` y `EditionDetail` en mobile orden 1-Edición 2-Título 3-Portada 4-Descripción 5-Botones (grid `grid-cols-1 md:grid-cols-2` con `md:col-start/row-start`, DOM = orden visual mobile, portada `md:row-span-3/4` a la derecha en desktop; título/descripción/botones `text-center md:text-left` + `justify-center md:justify-start`). “Tus tomos” carrusel: `relative px-10 md:px-14` gutter, flechas solo `‹ ›` `text-2xl md:text-3xl text-triba-brown hover:text-triba-red` sin fondo/borde, `gap-3 md:gap-5`, sección con `shadow-[inset_0...]` + gradients `from-black/10` top/bottom en el contenedor de sección (límite con otras secciones, no sobre portadas). Pills idioma en “Tus tomos”/hero/detalle: `badge` blanco si existe, si `lang==='en' && isFallback` → `EN coming soon` gris (`triba-bone`), si `lang==='en' && hasEn && !badge` → `EN` verde; payload `editionsPayload` incluye `badge/hasEn/isFallback` + `miCuentaMessages: {lang, comingSoon, badgeEn}`.
...
## Deuda de tipos
**`npx astro check` → 0 errores · `npm run build` OK** (22-Ago-2026, Bilingüe + mobile mi-cuenta/detalle 1-5 + pills idioma `badge/EN/EN coming soon` + badge único en #4).
